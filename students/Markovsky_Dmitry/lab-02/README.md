...
|-----|----------|------------|
| **Входящий порт** | CreateDealUseCase | Интерфейс для создания сделки |
| **Входящий порт** | GetDealUseCase | Интерфейс для получения сделки по ID |
| **Исходящий порт** | DealRepository | Интерфейс для хранения и загрузки сделок |
| **Исходящий порт** | InvoiceService | Интерфейс для создания инвойсов |
| **Исходящий порт** | NotificationService | Интерфейс для отправки уведомлений |
| **Входящий адаптер** | DealController (REST) | REST API для клиентов (входящие HTTP запросы) |
| **Исходящий адаптер** | InMemoryDealRepository | Хранилище сделок в памяти (для разработки/тестов) |
| **Исходящий адаптер** | MockInvoiceService | Имитация сервиса инвойсов (генерация mock-ссылок) |
| **Исходящий адаптер** | ConsoleNotificationService | Вывод уведомлений в консоль (вместо email) |
