...
<img src="./diagrams/test_deals.png"></img>
...
<img src="./diagrams/test_aggregate.png"></img>
...
<img src="./diagrams/test_events.png"></img>
...
В ходе выполнения работы реализован доменный слой для Deal Service с применением DDD-паттернов. Созданы 4 Value Objects (Money, DealStatus, ClientId, DealTitle) с валидацией и иммутабельностью. Реализованы 2 Entity (Deal, Invoice) с инвариантами и методами изменения состояния. Построен Aggregate Root DealAggregate, инкапсулирующий логику создания инвойса и оплаты. Добавлены 4 доменных события, регистрируемые при изменении состояния агрегата. Написаны юнит-тесты, покрывающие все бизнес-правила.

Что получилось: Чистое разделение бизнес-логики и технических деталей. Домен не зависит от фреймворков и БД. Все инварианты защищены.

Проблемы: Пришлось переосмыслить границы агрегата — изначально хотел включить Invoice в Deal, но понял, что это отдельная сущность в рамках одного агрегата.
